...
- (void)setItem:(DSImageScrollItem *)item {
    ...
    [_imageView setImageWithURL:item.largeImageURL placeholder:item.thumbImage options:YYWebImageOptionAvoidSetImage progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        __strong typeof(wself) sself = wself;
        if (!sself) return ;
        CGFloat progress = receivedSize / (float)expectedSize;
        progress = progress < 0.01 ? 0.01 : progress > 1 ? 1 : progress;
        if (isnan(progress)) progress = 0;
        wself.progressLayer.strokeEnd = progress;
        
    } transform:nil completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
        __strong typeof(wself) sself = wself;
        if (!sself) return;
        wself.progressLayer.hidden = YES;
